  ...
  return (
    <div
      className={`fixed z-50 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]
        ${scrolled ? "top-4 left-[16%] right-[16%]" : "top-3 left-6 right-6"}`}
    >
      <nav
        ...
        style={scrolled ? {
          background: "linear-gradient(135deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.03) 40%, rgba(120,120,180,0.06) 70%, rgba(255,255,255,0.05) 100%)",
          boxShadow: "0 8px 32px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.1), inset 0 -1px 0 rgba(255,255,255,0.03)",
        } : {
          background: "#18181b",
      ...
      >
        ...
        >
          ...
          <a href="/" className="flex items-center gap-2 shrink-0">
            <div
              className={`relative transition-all duration-500 ${scrolled ? "w-7 h-7" : "w-9 h-9"}`}
              style={{
            ...
            <span className={`text-white font-bold transition-all duration-500 ${scrolled ? "text-base" : "text-lg"}`}>
              AgenFast
            </span>
          </a>
          ...
          <div className={`hidden md:flex items-center transition-all duration-500 ${scrolled ? "gap-1" : "gap-1"}`}>
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="relative px-3 py-1.5 text-sm text-zinc-400 hover:text-white transition-colors duration-200 rounded-lg hover:bg-white/[0.06] group"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="md:hidden border-t border-white/[0.08] px-5 py-4 flex flex-col gap-1">
            {navLinks.map((link) => (
              <a
                ...
                className="py-2 px-3 text-sm text-zinc-300 hover:text-white hover:bg-white/[0.06] rounded-lg transition-colors"
              >
                {link.label}
              </a>
            ))}
